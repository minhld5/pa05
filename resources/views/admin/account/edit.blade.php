...
<div class="tab-pane" id="tab_edit_account">
	<form class="form-horizontal" action="{{@Config::get('app.url')}}/admin/account/edit/{{$user->userid}}" method="post">
		{{csrf_field()}}
		<div class="col-md-12">
			<div class="widget">
							...
							<div class="form-group">
								...
								<div class="col-md-8">
									<select class="form-control" name="isldap">
										<option value="0" <?php if($user->isldap=='0'){echo 'selected';} ?> >Local</option>
										<option value="1" <?php if($user->isldap=='1'){echo 'selected';} ?> >Ldap</option>
									</select>
								</div>
							</div>
								...
								<div class="col-md-8">
									<select class="form-control" name="role">
										@foreach($roles as $role)
											<option value="{{$role->id}}" <?php if($user->roleid==$role->id){echo 'selected';} ?>>{{$role->description}}</option>
										@endforeach
									</select>
								</div>
							</div>	
							...
							<div class="form-group">
								...
								<div class="col-md-8">
									<select class="form-control" name="unit" onchange="this.form.submit()">
										...
										@if (is_null($selectedunit) || ($selectedunit==''))
											@foreach($units as $unit)
												<option value="{{$unit->id}}"  <?php if($user->unitid==$unit->id){echo 'selected';}?> >{{$unit->name}}</option>
											@endforeach
										@else
											@foreach($units as $unit)
									...
									<select class="form-control" name="department">
										@if (!is_null($departments))
											...
											@foreach($departments as $department)
												<option value="{{$department->id}}" <?php if($user->departmentid==$department->id){echo 'selected';}?> >{{$department->name}}</option>
											@endforeach
										@endif
									</select>
								</div>
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<div class="col-md-8"><input type="text" name="email" class="form-control" value="{{$user->email}}"></div>
							</div>

							<div class="form-group">
								<label class="col-md-4 control-label">Tên đầy đủ:</label>
								<div class="col-md-8"><input type="text" name="fullname" class="form-control" value="{{$user->fullname}}"></div>
							</div>
